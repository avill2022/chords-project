...
class figures:
    def __init__(self,matriz_template):
        self.index = 0
        self.matriz = matriz_template
        ...
        for i, row in enumerate(self.matriz):
            ...
            if vals:
                ...
                if rmin < min_val:
                    min_val = rmin
                    min_idx = i
        self.matriz = self.matriz[min_idx:] + self.matriz[:min_idx-1]
    ...
    def show(self):
        note = CHROMATIC_SCALE[self.index-1]
        for row in self.matriz:
